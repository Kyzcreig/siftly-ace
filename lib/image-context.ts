...
export function buildImageContext(rawImageTags: string | undefined): string {
  ...
  try {
    ...
    if (typeof parsed.vision_caption === 'string' && (parsed.vision_caption as string).trim())
      parts.push(`Caption: ${(parsed.vision_caption as string).trim().slice(0, 200)}`)
    if (typeof parsed.video_transcript === 'string' && (parsed.video_transcript as string).trim())
      parts.push(`Transcript: ${(parsed.video_transcript as string).trim().slice(0, 400)}`)
