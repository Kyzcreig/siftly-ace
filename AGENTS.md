...
## Long ("note") tweet full text — react-tweet caps at 280 (2026-06-24)
- `react-tweet getTweet(id)` returns only the **280-char** legacy `t.text` for LONG/note tweets; the `note_tweet` field it exposes carries only an id, NOT the body. So the HTML report (which hydrates via react-tweet) truncated long tweets mid-sentence.
- The FULL body lives in the X API `note_tweet.text` (verify: `xurl --app forge '/2/tweets/<id>?tweet.fields=note_tweet'`). Gather's `lib/twitter-api.ts tweetFullText()` already reads it.
- FIX (shipped): x-feed Step 6.7 dump now stores full `tweet_text` (alongside the ≤120 `text_snippet`; the 120 cap was only ever for the pf-audit artifact, not this local debug dump). `html_report.ts renderItem` prefers the stored full `tweet_text` over react-tweet's hydrated text when longer (strips trailing media t.co; tr-tag only shows when a real srcLang is set). morning dump already stored full tweet_text.
- DISCIPLINE: proving a render/length fix → pull the REAL tweet via xurl and prove on real bytes. Do NOT hand-write fake "after" demo text (Ace caught a fabricated PyTorch example — the real tail was "video inputs and a 256K context window. Read more:", ~356 chars, not the invented paragraph I showed).


## Brief UX & quality batch — SHIPPED 2026-06-24 (live next 03:33/03:48 run)
A batch of Ace-requested brief-quality fixes; all behind the dark-flag discipline, npm run verify green, both prompts live + snapshotted to `deploy/cron-prompts/`.
- **Reddit low-signal demotion (`score_digest.py is_reddit_low_signal` + Backstop 3):** thin-body / vague-one-liner reddit chatter (no real writeup) demoted to `context_only` so forum venting can't sit at ALSO_GATE and fill Also-Noted with stubs. Body is the primary signal; developed questions rescued. REDDIT-ONLY (never touches github/HN/X). Named constants `REDDIT_LOW_SIGNAL_*`. Engagement-based demotion was impossible — the reddit RSS pivot carries no upvote/comment counts. Selftest teeth-tested.
- **Unified footer (`scripts/footer_build.py`, Candidate A):** ONE deterministic 3-line footer both briefs call (was hand-composed prose per-brief -> drifted). Line1 scanned/new/filtered, line2 sources (nonzero, sorted desc, thousands-sep), line3 deterministic scoring + personal-fit. morning lists 7 content sources; x-feed lists Timeline+Search channels. Fail-safe. `html_report.ts` renders the footer newlines as `<br>` (3 stacked lines). Both prompts build it from real counts + overwrite `.footer` before posting.
- **Overview clean labels (`overview_digest.py _label()`):** top_stories[].label + theme examples now emit a clean `@handle: gist` (leading reply @mentions stripped) for tweets / clean title-slug for non-tweets — was feeding the LLM raw truncated tweet TEXT as story "names" -> junk theme chips. Both prompts: name stories by `label`, write theme paragraphs themselves (never paste example fragments), Title-Case themes.
- **Story summary 300-cap / tweets unlimited:** morning summary formula + debug-dump cap lifted 120->300 chars (1-3 sentences, no trailing ellipsis); `html_report.ts linkCard` hard-caps story summaries at 300 at a word boundary; tweets never length-capped.
- **Auto-translate (option B, `lib/translate.ts`):** non-English item text -> English, REPLACE + x.com-parity tag `Translated from <Language> · Show original` (link = the original x.com post / story url; 2026-07-05). Detection: cheap local script regex (CJK/Cyrillic/etc.; JP detected before Han so Japanese != Chinese) PLUS `detectLatinForeign()` stopword heuristic for French/Spanish/German/Portuguese/Italian (>=4 distinctive hits, 2x over English, >=12% density — English never hits the API). Labels are English language names. One short LLM call via repo AI client; prefers OpenAI when OPENAI_API_KEY in env (build-report.sh sources with-secrets.sh). FAIL-SAFE: any error -> original text; `SIFTLY_TRANSLATE=0` disables. Wired into tweet-card body AND story TITLE+SUMMARY render paths (`renderItem` translates both; `linkCard` takes a `trTitle` arg, renders the English headline + one tag). Tests: `lib/__tests__/translate.test.ts` (19). **2026-07-06 fix (was silently DEAD for BOTH briefs since the ~Jul-1 node upgrade):** `translateToEnglish()` looked up the model via `getActiveModelFor()`, a Prisma/better-sqlite3 DB call that THROWS under the current node (native-module ABI mismatch, NODE_MODULE_VERSION 147 vs 127); the fail-safe swallowed it → nothing ever translated. `getModelSafe()` now falls back to a per-provider default model (`gpt-4o-mini` for openai) when the DB lookup throws, so a broken/mismatched DB binding can't disable translation. Also fixed: story TITLES were never translated (only summaries) — the morning-digest-visible gap (story-heavy brief, headline IS the content). **ROOT ENV FIX (also 2026-07-06):** the native module was built for the old node ABI (147) and node got upgraded to v22 (modules 127) without a rebuild, so ALL TS Prisma/better-sqlite3 DB paths (settings lookups, embeddings, the webapp) threw. **This now self-heals automatically** — `scripts/ensure-native-abi.sh` is the ONE self-heal authority (consolidated 2026-07-06 from two copies — the old inline block in `daily-ingest.sh` per commit `2f0b1de` + a second copy on the brief path — DRY'd into one). It probes better-sqlite3's native load and runs `npm rebuild` ONCE if it detects the ABI-mismatch signature (never on an unrelated load failure). Two fail-modes via `--strict`: `daily-ingest.sh` calls it **`--strict` (fail-closed, exit 1)** — the DB is required, a broken rebuild aborts the run; `build-report.sh` Step 0a calls it **default (fail-open, exit 0)** — the DB is optional (translation degrades to original text), never block the nightly brief. Also wired into `package.json postinstall`. So no human has to remember to rebuild after a node upgrade (cheap on the happy path ~38ms, narrow). `node_modules` is gitignored so the rebuilt binary isn't committed, but the self-heal guard IS committed, so a fresh checkout or an upgraded node recovers on the next brief run / ingest / install.
- **Landscape overview 4x longer:** `inject_overview.py MAX_CHARS` 1800->7200; both prompts' Step 6.9 rewritten from half-page/one-line-bullets to a ~1.5-2pg briefing (lead paragraph + labelled paragraph per theme, 6-10 cited stories). Anti-boilerplate kept absolute.
- **Long ("note") tweet full text:** both briefs' X fetch now requests `tweet.fields=...,note_tweet`; agent captures `note_tweet.text ?? text` (v2 FLAT shape, NOT lib's tweetFullText() GraphQL shape); `html_report.ts renderItem` prefers the stored full tweet_text over react-tweet's 280-char hydrated text. Verified live: PyTorch tweet 280->356, another 295->577.
