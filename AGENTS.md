...
- **Wikipedia-style story refs (2026-06-22):** the overview prose cites named stories as `[N]`, matching the `ref` numbers `overview_digest.py` now stamps on each `top_stories` item (with a guaranteed `url`). `scripts/resolve_overview_refs.py` maps the cited `[N]` → real URLs and appends a `🔗 [1] <url> · [2] <url>` line. **URLs come from the deterministic aggregate, NEVER the LLM** — a citation can't point at a hallucinated link; bogus `[N]` are silently dropped; no citations → no refs line. Both prompts run resolve before inject/compose; fail-safe passthrough. `.bak.*-pre-reflinks` backups.
