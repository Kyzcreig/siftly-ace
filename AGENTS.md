...
## Reddit gatherer — datacenter-IP 403 (FIXED, needs creds), commit f4af53a
- ROOT CAUSE: Reddit blocks UNAUTHENTICATED .json reads from datacenter/non-residential IPs. Mac Studio host = 403 on every UA + every egress lane (Starlink too). The access_token endpoint = 401 (IP fine for AUTH, only anon reads blocked). UA spoofing / egress-hopping do NOT fix it.
- FIX: scripts/gather/reddit.ts now does app-only OAuth (cached token, oauth host, Bearer) when REDDIT_CLIENT_ID/SECRET are set; graceful anon fallback otherwise (returns [], no crash). with-secrets.sh loads creds from 1Password (refs point at item "REDDIT_APP_ITEM" — UPDATE to real item id after registration).
- REMAINING ONE-TIME (Ace): https://www.reddit.com/prefs/apps -> create app, type "script" -> store client_id + secret in 1Password Engineering -> update the two op:// refs in with-secrets.sh. Then anon 403 -> 200 via oauth host.
- github-trending gatherer = healthy (HTTP 200, ~14 net-new/run). Reddit was the only blocked source.
- Live proof: dummy-cred probe took oauth branch, token endpoint returned 401 (not 403). Anon run logged "HTTP 403; returning []" and exited 0.
