...
- **HTML report delivery (ask 3 — UNDECIDED, awaiting Ace):** a doc-share HTML version of a brief was sampled (`rising-quasar-2prv.here.now`) for comparison vs inline Discord. Apollo's rec = keep inline as the brief + optionally auto-attach an HTML link in the footer (glanceable + archive). NOT built — pending Ace's pick. If chosen: render markdown → promote headers (`#`/`##`/`###`) → `doc-share.sh` → footer link. Do NOT replace the inline post without his explicit go.
