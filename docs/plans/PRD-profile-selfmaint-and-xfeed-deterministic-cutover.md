...
<li><strong>P2.4 (Hard-Config gate) — DONE 2026-06-11 (Ace-approved, applied live):</strong> flipped x-feed live render to the deterministic engine. Step 6 now runs <code>select_digest.py --engine deterministic --max-top 5 --max-also 5 --top-gate 60 --also-gate 50</code> over the Step-6.7 dump (legacy-prose fallback if it errors/empties). Step 6.75 shadow retired; Step 5b note updated (labels now DRIVE posting). Gates stay 60/50 (P2.3 proved they don't bind — ranking+dedup are the levers). Live-verified end-to-end on labeled data: 5 Top + 5 Quick Hits, all renderable, political/health junk dropped. Backup <code>~/.hermes/state/cron/x-feed-brief/prompt.md.bak-20260611-153244-pre-p24-flip</code>; rollback = restore backup (engine code + additive labels stay).</li>
</ol>
