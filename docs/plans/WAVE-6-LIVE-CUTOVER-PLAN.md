...
**Daily side-by-side preview (Ace's call 2026-06-18 — judge by eye, not by %):** no_agent cron `siftly-pf-preview` (job `54ca29c4343e`, daily **4:15am PT** — AFTER the briefs finish at 3:33am/3:48am, NOT 8am; the original 8am left a 4h "brief but no shadow" gap, fixed 2026-06-20), script `~/.hermes/scripts/siftly-pf-preview.py`) renders BOTH briefs keyword-vs-embed from each morning's real pool and posts a compare card to the #daily / Twitter-Bookmark-Job thread (`1513211118573584495`). PREVIEW ONLY — never posts to #daily, never mutates seen-lists/render_input/brief state; reads pools + pf-audit, writes only /tmp + one Discord message. The post path now checks `notify.py`'s exit code and returns non-zero on a failed post (was silently exiting 0). Morning uses the real `render_digest.py` contract; x-feed uses a ranked-list (no deterministic renderer exists for it). Purpose: accumulate a few days of eyeball comparisons, then Ace picks **refinement** (recalibrate `PF_BASELINE`, keep churn ≤10–20%) / **taste-forward** (accept churn, change the gate) / **hybrid** (embed → Also Noted first) / **stop**.
