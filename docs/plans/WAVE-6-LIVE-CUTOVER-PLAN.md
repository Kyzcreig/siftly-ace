...
**FUSED mode (Ace's "use BOTH scoring systems, re-rank to include both" — 2026-06-20):** Ace judged keyword and embed equally good (a lateral 73% churn → no winner). Resolution is rank fusion, not a flip: `fused_pf = (keyword_delta + (embed_delta − pool_mean_embed)) / 2`. Centering embed by the pool mean cancels its miscalibrated ~+10.6 offset (so NO `PF_BASELINE` recalibration needed first) while preserving its ordering (centering is monotonic); keyword is already ~centered (mean ~+0.4). The average stays on keyword's scale so `pf_points`/`PF_CAP`/`select_shadow` consume it unchanged, and gives both scorers equal say (an item ranks high only if both like it, or one likes it strongly). Currently shadow-only in the preview's 3rd column. If Ace says **"fuse it"**, the live build = compute `fused_pf` in `pf-score.py`/`pf-audit.py` and feed it as `personal_fit_delta` (a one-helper change, then the existing gated `PF_AFFINITY_MODE`-style flip) — bring diff + dry-runs for go-live approval. Other replies: **keep keyword** / **wait** (let the ~Jun 28 saved-item provenance decide) / **stop**.
