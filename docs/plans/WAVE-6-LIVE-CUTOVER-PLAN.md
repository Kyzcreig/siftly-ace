...
**Daily side-by-side preview (Ace's call 2026-06-18 — judge by eye, not by %):** no_agent cron `siftly-pf-preview` (job `54ca29c4343e`, daily **4:15am PT** — AFTER the briefs finish at 3:33am/3:48am, NOT 8am; the original 8am left a 4h "brief but no shadow" gap, fixed 2026-06-20), script `~/.hermes/scripts/siftly-pf-preview.py`) renders BOTH briefs **keyword vs embed vs FUSED** from each morning's real pool and posts a compare card to the #daily / Twitter-Bookmark-Job thread (`1513211118573584495`). PREVIEW ONLY — never posts to #daily, never mutates seen-lists/render_input/brief state; reads pools + pf-audit, writes only /tmp + one Discord message (post path checks `notify.py` exit code, returns non-zero on failure). Morning uses the real `render_digest.py` contract; x-feed uses a ranked-list.

**FUSED mode — SHIPPED + LIVE (Ace "ship fused, fuse it" — 2026-06-20):** Ace judged keyword and embed equally good (a lateral 73% churn → no winner). Resolution is rank fusion, not a flip: `fused_pf = (keyword_delta + (embed_delta − pool_mean_embed)) / 2`. Centering embed by the pool mean cancels its miscalibrated ~+10.6 offset (so NO `PF_BASELINE` recalibration needed) while preserving its ordering (centering is monotonic); keyword is already ~centered. The average stays on keyword's scale so `pf_points`/`PF_CAP`/`select_shadow` consume it unchanged, giving both scorers equal say.
- **Code:** `pf-score.py` gained `affinity_mode="fused"` + `apply_fused_affinity()` (pool-level — needs the whole set for the mean; back-derives `personal_fit_raw=fused/weight`; preserves `keyword_*` fields for audit). `pf-audit.py` `_affinity_mode_wants_embed()` now provisions the embed env for `fused` too (else it'd silently keyword-fall-back). Real-vec formula test added (`pf-score-embed.test.ts`, 2-candidate pool so centering is non-trivial). Commit pushed; `npm run verify` green (TS+py+e2e, lint 0).
- **Gates proven before go-live:** ≥3 deterministic dry-runs; formula byte-matches the preview; `PF_WEIGHT=0` kill-switch → `base_score_only`/`sentinel`, all deltas 0; failure-isolation → embed broken → `keyword_fallback`, `ok:true`, brief survives.
- **Live flip (gated, Ace-approved this turn):** both `prompt.md` PF calls flipped `PF_AFFINITY_MODE=shadow`→`fused` (morning-digest L273, x-feed-brief L87). Timestamped `.bak.<ts>-pre-fused` backups exist. **Rollback** = restore the `.bak` (or sed `fused`→`shadow`), or kill-switch `PF_WEIGHT=0`. End-to-end live-path verified: pf-audit fused via with-secrets → `ok:true, affinity_source=fused, vec_metric=l2norm`, 15/15 nonzero deltas, audit artifact `fired:true affinity_mode:fused`. Takes effect next brief run (3:33/3:48am).
- The daily `siftly-pf-preview` keyword/embed/FUSED card keeps running so the live fused brief stays watchable against both pure modes.

**Jun 29 "are we done?" check-in (Ace's call 2026-06-20):** one-shot no_agent cron `siftly-fused-checkin` (job `5366991ff190`, fires once 2026-06-29 09:00 PT, script `~/.hermes/scripts/siftly-fused-checkin.py`, self-removes after firing per Rule #5a). Posts ONE decision card to the #daily thread with REAL data: (1) **provenance** — of brief-surfaced items matured ≥14d (cohort from Jun 13–15 matures Jun 27–29), the % Ace later actually saved (matches surfaced tweetId → `Bookmark.tweetId` with `importedAt ≥ surfaced day`; matcher verified on real data: forced-now=Jul1 → 60 matured / 2 saved / 3.3%); (2) **fused-vs-keyword gate-cross** on the latest pool (reuses `pf_gatecross_eval.py`); (3) **what's safe to retire** once fused is kept (`wave6-embed-shadow-watch`, `wave6-output-shadow-watch`, eventually `siftly-pf-preview`, the paused coercion watch, the pre-fused `.bak`s). Reply options: keep fused / tune / revert / wait. This is the natural project-closeout decision point — data, not vibes. (Read-only; re-exec guard verified under anaconda 3.7.)
