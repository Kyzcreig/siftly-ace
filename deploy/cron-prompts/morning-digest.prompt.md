...
```
It prints a one-line audit (`pool=… → top=… also=… | dropped bare=… below_gate=… | boost-gated off-topic=…`). Do NOT hand-edit `_render_input.json` after this — the guard owns its selection structure. (The scripted Overview, footer, and delta steps below add presentation-only fields; none may alter membership, order, or scores.) If the guard exits non-zero, that is the real failure; surface it (do not hand-write the render input).
...
```
This is fail-safe: any error leaves the existing footer untouched.

**Next, apply the deterministic yesterday-delta presentation layer** (after selection and all additive fields, before either renderer):
```bash
python3 ~/Projects/siftly-ace/scripts/brief_delta.py --current ~/.hermes/state/cron/morning-digest/_render_input.json --state ~/.hermes/state/cron/morning-digest/brief-delta-history.json --out ~/.hermes/state/cron/morning-digest/_render_input.json --date "$(TZ=America/Los_Angeles date +%F)" --defer-snapshot || echo "brief delta failed — rendering today's selection unchanged" >&2
```
The script keeps dated structured snapshots, compares only an exactly consecutive PT-day brief, and adds `_delta`/`delta` presentation metadata. It does **not** remove, reorder, rescore, or replace anything in `selected`/`also`; both renderers collapse items marked `unchanged`, lead visible items with `new`/`moved`/`resolved`, and show prior-only resolved items separately. A missing baseline starts a new one instead of claiming stale items resolved. `--defer-snapshot` prevents a failed post from becoming tomorrow's false "yesterday" baseline; the post block commits the snapshot only after delivery succeeds. Any apply error leaves the render input unmodified and the brief still posts.

Then build the report and post its link:
```bash
...
if [ -n "$REPORT_URL" ]; then
  python3 ~/.hermes/scripts/notify.py --send "☀️ **Morning Digest** — $(date '+%A, %B %-d') → $REPORT_URL"$'\n'"$(jq -r '.footer // empty' ~/.hermes/state/cron/morning-digest/_render_input.json 2>/dev/null)" --channel discord --target 1480539453117305023 && POSTED_OK=1
else
  ...
  python3 ~/Projects/siftly-ace/scripts/render_digest.py --in ~/.hermes/state/cron/morning-digest/_render_input.json --post --no-dedup --target 1480539453117305023 && POSTED_OK=1
fi
if [ "$POSTED_OK" -eq 1 ]; then
  python3 ~/Projects/siftly-ace/scripts/brief_delta.py --snapshot-only --current ~/.hermes/state/cron/morning-digest/_render_input.json --state ~/.hermes/state/cron/morning-digest/brief-delta-history.json --date "$(TZ=America/Los_Angeles date +%F)" || echo "brief posted but delta snapshot commit failed" >&2
else
  echo "morning digest post failed — delta snapshot not advanced" >&2
  false
fi
```
