...
```
It emits `{themes:[{topic,count,salience,examples}], top_stories:[{ref,label,title,handle,content_type,final_score,url}], loud_authors, content_mix, pool_size, on_topic_size, off_topic_count}`. **Use the `label` field (a clean `@handle: gist` or repo/headline name) when you name a story — NEVER paste the raw `title`/tweet text.** From THAT aggregate, compose the synthesis and write it to `/tmp/morning-digest-overview.txt`. Format for communication, roughly half a page (≤1800 chars):
- Open with a header line: `🗞️ **The Landscape**`
- **2–4 sentences** naming the day's BIG THEMES (use the top `themes` by salience) and BIG STORIES (the top few `top_stories` by `final_score`) woven together — refer to each story by a **short human paraphrase built from its `label`** (e.g. "Anthropic's Claude Tag Slack agent", "a minimal AI-coding terminal, kooky"), name specific models/tools/people/numbers, NOT vibes and NOT raw tweet fragments. The goal is "if I read only this, I know what happened in AI today."
- **CITE stories with `[N]` markers** — when you mention a specific story, append its `ref` number from `top_stories` in square brackets, e.g. "Claude Tag shipped as an always-on Slack agent [4]". Use ONLY the integer `ref` values that exist in the aggregate; cite each at most once; do NOT write URLs yourself (a script appends the real links). Cite the 3–6 stories you actually name.
- Then **2–5 short theme bullets** with the count, each one line, where the dash-part is a **genuine one-line characterization of that theme you write yourself** — NOT a pasted example fragment. Good: `• **Agent tooling** (65) — Claude Tag routes 65% of Anthropic's internal commits; kooky ships a minimal coding terminal`. Bad: `• **Ai Ml** (119) — @Grok: Daily Claude AI News Briefing…` (raw fragment — never do this). The `examples` in the aggregate are RAW SOURCE you summarize from, never text to copy verbatim. Pick the themes that actually have stories behind them; merge near-duplicate topics (ai-ml/dev-tools/agents-coding overlap — collapse to human themes like "Models", "Agent tooling", "Coding", "Security") and Title-Case them nicely (not "Ai Ml").
