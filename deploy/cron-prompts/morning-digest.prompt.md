...
- **A lead paragraph (2–4 sentences)** naming the day's BIG THEMES (top `themes` by salience) and BIG STORIES (top `top_stories` by `final_score`) — refer to each story by a **short human paraphrase built from its `label`** (e.g. "Anthropic's Claude Tag Slack agent", "a minimal AI-coding terminal, kooky"), with specific models/tools/people/numbers, NOT vibes and NOT raw tweet fragments. The goal: "if I read only this, I know what happened today."
  - **You MUST write ACTUAL SENTENCES, not a list of labels glued together.** Do NOT paste `@handle: gist` labels verbatim into a sentence and connect them with filler. ❌ BAD (this is a banned shape): *"Models, Coding set the pace today: @velonxbt: Mecha Systems launched the Mecha Comet [1]; @CryptoTice_: BREAKING: The U.S. [2], with @gulVasikova: $COIN U.S. [4] as the useful secondary read."* — that is label-pasting, not writing. ✅ GOOD: *"The U.S. cleared Claude Mythos 5 for ~100 vetted orgs after a two-week blackout, hardening the case for open weights [2]. On the homelab side, a $3K 24-bay tower that mirrors every open model made the rounds [5]."* Write what HAPPENED in your own words; the `[N]` just attaches the source. If a `label` is foreign-language, a bare ticker ("$COIN U.S."), or too thin to paraphrase into a real claim, DROP it — never surface it.
- **A junk filter for the lead/bullets:** the aggregate is ranked by an upstream score that sometimes over-rates crypto-ticker/Kickstarter/foreign-language/clickbait posts. Before you name a story, sanity-check it is genuinely about AI/models/agents/building. If the top `top_stories` are a crypto gadget or a `$TICKER` post, SKIP them and reach down to the first items that are real AI news — a short overview of 2 real stories beats 4 with junk in it.
...
- **Then 2–4 ONE-LINE theme bullets** — `• **Theme** — <one line of real content>`. Each bullet is a single sentence of actual substance (what shipped, who, the number), NOT a paragraph. Merge near-duplicate topics into human themes ("Models", "Agent tooling", "Coding", "Security") and Title-Case them (not "Ai Ml"). **If a theme has nothing concrete to say, DROP it — never pad to reach a count.**
