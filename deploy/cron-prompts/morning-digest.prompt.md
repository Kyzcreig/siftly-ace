...
**🎯 THE WHOLE POINT (read this first): the Landscape is a map of the ENTIRE day's sampled pool (`pool_size` items across `on_topic_size` on-topic), NOT a recap of the ~10 stories that got posted.** Ace already sees the Top Stories below; the Landscape's job is to tell him *what the whole field was doing* — which lanes were busy (`themes` with their `count`s), what the overall mix was (`content_mix`: launches vs analysis vs news), who was loud (`loud_authors`), and what's bubbling UNDER the headlines, not just the headlines. Use the `count`/`content_mix`/`pool_size` numbers — they are the breadth signal. A Landscape that only names the top stories has failed its job.

- Open with a header line: `🗞️ **The Landscape**`
- **A lead paragraph (2–4 sentences) that maps the DAY, not the top stories.** Open with the *shape of the field* — how much moved and where the gravity was, using the real theme `count`s and `content_mix` (e.g. "A heavy build-day — coding agents (63), agents (59) and models (57) dominated a 169-item field that skewed toward things that shipped, not think-pieces"). THEN name the 1–2 genuinely biggest events as anchors. Model the voice on x-feed's "Your Timeline": a flowing read of the whole landscape, NOT a roll-call of posts. Name specific models/tools/people/numbers, paraphrased into real claims; `@handle`/`[N]` ride *inside* the sentence.
