...
**🎯 THE WHOLE POINT (read this first): the Landscape is a map of the ENTIRE day's sampled pool (`pool_size` items across `on_topic_size` on-topic), NOT a recap of the ~10 stories that got posted.** Ace already sees the Top Stories below; the Landscape's job is to tell him *what the whole field was doing* — which lanes were busy (the `themes`, already merged + cleanly named for you), what the overall mix was (`content_mix`: launches vs analysis vs news), who was loud (`loud_authors`), and what's bubbling UNDER the headlines. **Weave the breadth in as a READ, not a readout.** A count or two, dropped naturally inside a sentence, is good ("agents were the busiest lane"); a bare stat-dump opener is NOT — 🔴 BANNED (happened 2026-06-30): *"A 225-item sample (196 on-topic, 29 off-topic) leaned into models (109), agents (63), dev tools (42); the mix skewed toward analysis 100, launch 72, tutorial 9."* That's a spreadsheet, not writing. Lead with what MOVED and let the numbers ride inside real sentences. A Landscape that only names the top stories has also failed its job — cover the field, but as prose.

- Open with a header line: `🗞️ **The Landscape**`
- **A lead paragraph (2–4 sentences) that maps the DAY, not the top stories.** Lead with what actually MOVED — the 1–2 genuinely biggest events, in your own words — then place them against the shape of the field (which lanes were busiest, whether the day skewed shippers vs think-pieces), letting a count or two ride *inside* the sentence rather than opening with a stat line. ✅ GOOD (story-first, numbers woven in): *"Opus 4.8 landing was the day's gravity well [1], but the real volume was in agent-building — the busiest lane by far — with a recurring prompt-injection-hardening thread running underneath [2]. A build-day, not a think-piece day."* ❌ BAD (stat-dump opener): *"A 225-item sample (196 on-topic, 29 off-topic) leaned into models (109), agents (63)…"*. Model the voice on x-feed's "Your Timeline": a flowing read of the whole landscape, NOT a roll-call of posts. Name specific models/tools/people, paraphrased into real claims; `@handle`/`[N]` ride *inside* the sentence.
...
- **Then 3–5 ONE-LINE theme bullets that COVER THE FIELD** — `• **Theme** — <what this lane was about today + how busy it was>`. These are the breadth layer: walk the dominant `themes` (by `count`) and say what each lane was actually doing, so Ace sees the spread UNDER the headlines, not just the top 2 stories repeated. Weave in the activity level naturally ("the busiest lane", "a quieter but sharp thread on…"). Each bullet is a single WRITTEN sentence, NOT a raw `label`/`example` pasted in. 🔴 BANNED (happened 2026-06-30): `• **Coding** — 🔥Latest Claude Updates (June 2026) Claude Opus 4.8 Launch…` — that is a pasted fragment, not a sentence. ✅ `• **Agents** — the day's biggest lane: people stress-testing Opus 4.8 on SWE-bench and arguing about Cursor-style autonomous agents.` **The `themes` are ALREADY merged and cleanly named for you (`topic` = "Agents", "AI/ML", "Dev Tools" — use it VERBATIM as the bold label; do NOT re-case, re-merge, split, or abbreviate it — that's how you get garbage like "Ai MI").** Just write the one-line description after it. Strip any leading emoji from source text. **Cover the genuinely active lanes; drop a theme only if it has nothing concrete — but don't collapse to just 2 when the field had 6 busy lanes.**
