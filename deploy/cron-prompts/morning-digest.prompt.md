...
```
It emits `{themes:[{topic,count,salience,examples}], top_stories:[{ref,label,title,handle,content_type,final_score,url}], loud_authors, content_mix, pool_size, on_topic_size, off_topic_count}`. **Use the `label` field (a clean `@handle: gist` or repo/headline name) when you name a story — NEVER paste the raw `title`/tweet text.** From THAT aggregate, compose the synthesis and write it to `/tmp/morning-digest-overview.txt`. This is a **real briefing, ~1.5–2 pages (aim 3000–7000 chars, hard ceiling 7200)** — not a teaser. Ace reads ONLY this to know the day; make it worth reading:
- Open with a header line: `🗞️ **The Landscape**`
- **A lead paragraph (4–7 sentences)** weaving the day's BIG THEMES (top `themes` by salience) and BIG STORIES (top `top_stories` by `final_score`) — refer to each story by a **short human paraphrase built from its `label`** (e.g. "Anthropic's Claude Tag Slack agent", "a minimal AI-coding terminal, kooky"), with specific models/tools/people/numbers, NOT vibes and NOT raw tweet fragments.
- **CITE stories with `[N]` markers** — when you mention a specific story, append its `ref` number from `top_stories` in square brackets, e.g. "Claude Tag shipped as an always-on Slack agent [4]". Use ONLY the integer `ref` values that exist in the aggregate; cite each at most once; do NOT write URLs yourself (a script appends the real links). Cite 6–10 stories you actually name across the whole overview.
- **Then a labelled paragraph per major theme (3–6 themes)** — a real `**Theme.**` lead-in followed by 2–4 sentences that actually explain what happened in that lane (the launches, who shipped what, the numbers, the tension), written by YOU from the stories — NOT pasted example fragments. The `examples` in the aggregate are RAW SOURCE you summarize from, never copy verbatim. Merge near-duplicate topics (ai-ml/dev-tools/agents-coding overlap → human themes like "Models", "Agent tooling", "Coding", "Security") and Title-Case them (not "Ai Ml").
- Close with **1–2 sentences on the day's shape** (e.g. "Quiet on funding; heavy on open-weights; the agent-harness fight is clearly the week's throughline.").
- **Anti-boilerplate is STILL absolute** — no "the AI world is buzzing", no filler to hit a length. Every sentence carries a proper noun or a number. Longer means MORE real substance, not padding; if a day is genuinely thin, a shorter honest briefing beats a padded one.
