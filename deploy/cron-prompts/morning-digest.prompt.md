...
One sentence (max ~30 words), structured as:
...
**MANDATORY scored-debug dump (observability + the deterministic selection feed):** Immediately after scoring & before posting, write the FULL scored candidate set to `~/.hermes/state/cron/morning-digest/_last_run_debug.json` with `write_file` (overwrite each run). Schema: `{run_id, ts, pf_note, x_failure_note, selected:[...], also:[...], all_scored:[{title,url,summary(≤120 chars),source,published_at,tweet_id,authorHandle,authorName,likes,retweets,tweet_text,event_key,base_score,score_notes,personal_fit_raw,personal_fit_delta,final_score,personal_fit_fired,pf_weight,pf_baseline,signals,dropped_reason}]}`. Sort `all_scored` by `final_score` desc; EVERY scored candidate appears with a `dropped_reason` (`selected_top|also_noted|below_83|below_77|topic_dup|seen_dedupe`) so any run is reconstructable from disk.

**This dump is now the INPUT to deterministic selection (Step 6.7) — so for X items you MUST include the FULL verbatim `tweet_text` (not the ≤120-char snippet) and an `event_key` on every candidate**, exactly as you would have put them in the render JSON. (`base_score` is the pre-boost rubric score; `signals.topic_hits` is the topic vector. The selection guard re-derives boosts + selection from these — see Step 6.7.) For non-X stories keep the ≤120-char `summary` snippet. Do NOT store full article body text — only the tweet text and the short story summary.
...
```
It emits `{themes:[{topic,count,salience,examples}], top_stories:[{ref,title,handle,content_type,final_score,url}], loud_authors, content_mix, pool_size, on_topic_size, off_topic_count}`. From THAT aggregate, compose the synthesis and write it to `/tmp/morning-digest-overview.txt`. Format for communication, roughly half a page (≤1800 chars):
- Open with a header line: `🗞️ **The Landscape**`
- **2–4 sentences** naming the day's BIG THEMES (use the top `themes` by salience) and BIG STORIES (the top few `top_stories` by `final_score`) woven together — name specific models/tools/people/numbers, not vibes. The goal is "if I read only this, I know what happened in AI today."
- **CITE stories with `[N]` markers** — when you mention a specific story, append its `ref` number from `top_stories` in square brackets, e.g. "GLM-5.2 shipped as the strongest open coding model [4]". Use ONLY the integer `ref` values that exist in the aggregate; cite each at most once; do NOT write URLs yourself (a script appends the real links). Cite the 3–6 stories you actually name.
- Then **2–5 short theme bullets** with the count, each one line: e.g. `• **Open-weights models** (6) — GLM-5.2 tops open coding; Japan's Mythos-Fable rival`. Pick the themes that actually have stories behind them; merge near-duplicate topics (ai-ml/dev-tools/agents-coding overlap — collapse to human themes like "Models", "Agent tooling", "Coding", "Security").
- Optionally ONE closing line on the day's shape (e.g. "Quiet on funding; heavy on open-weights.").
- **Same anti-boilerplate rules as item summaries** — no "the AI world is buzzing", no filler. Every sentence must carry a proper noun or a number. If you can't write something genuinely informative, write a shorter honest version (even 2 sentences) rather than padding.
...
**A) TWEETS (`source: "X"`) — render VERBATIM, like the x-feed brief.** Do NOT summarize or paraphrase a tweet. Put the tweet's real text in `tweet_text` exactly as posted (keep line breaks, emojis, @mentions, t.co links). The renderer shows the author + engagement meta line, then the verbatim text, cutting at a natural sentence/line boundary if it's very long. NEVER write an "@handle flags …" prefix or a hand-written gloss for a tweet — paste the actual tweet.

**B) STORIES (`source: "HN" | "smol.ai" | "Latent Space" | "Perplexity"`) — headline + ONE distinct summary line.** `title` is a clean self-contained headline; `summary` is ONE additional line that ADDS information beyond the headline (NEVER a reprint/re-truncation of it — the renderer drops a summary that just echoes the headline). If you can't say anything beyond the headline, omit `summary`.
